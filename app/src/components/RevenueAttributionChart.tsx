...
export function RevenueAttributionChart() {
  ...
  const q = LATEST_ATTRIBUTION
  const rateChange = liveRate ? Math.round((liveRate.rate - q.reserveIncome.returnRate) * 10000) : null
  ...
  return (
    <div className="bg-white rounded-xl p-5 shadow-sm border border-gray-100">
        ...
        <div>
          ...
          <div className="flex items-center gap-3 mt-0.5">
            <p className="text-xs text-gray-500">
              {q.periodLabel}: ${(q.totalRevenue / 1e6).toFixed(0)}M 总收入 | RLDC {q.rldcMarginPct}%
            </p>
            {liveRate && (
              <span className="inline-flex items-center gap-1 text-xs font-medium px-2 py-0.5 rounded-full bg-blue-50 text-blue-700">
                ...
                {rateChange !== null && rateChange !== 0 && (
                  <span className={rateChange > 0 ? 'text-green-600' : 'text-red-500'}>
                    {rateChange > 0 ? ' ↑' : ' ↓'}{Math.abs(rateChange)}bps vs {q.periodLabel}
                  </span>
                )}
              </span>
            )}
      ...
      <p className="text-xs text-gray-400 mt-3">
        数据来源：SEC 10-Q Q1 2026 财报 + 财报电话会 | 产品费率为估算值
      </p>
    </div>
  )
}
...
function WaterfallView({ liveRate }: { liveRate: LiveRateData | null }) {
  ...
  const data = [
    { name: '储备利息（总）', value: q.reserveIncome.gross, fill: '#2962ff' },
    { name: 'Coinbase 分成', value: -q.reserveIncome.distributionCosts, fill: '#e91e63' },
    { name: '储备利息（净）', value: q.reserveIncome.net, fill: '#1565c0' },
    { name: '订阅/服务收入', value: q.otherRevenue.subscriptionServices, fill: '#f5ac37' },
    { name: '交易费收入', value: q.otherRevenue.transactionRevenue, fill: '#26a69a' },
  ]
    ...
    <>
      <div className="grid grid-cols-3 gap-3 my-4">
        <div className="rounded-lg bg-blue-50 px-3 py-2">
          ...
          <p className="text-lg font-semibold text-blue-700">{formatUSD(q.reserveIncome.gross)}</p>
          <p className="text-xs text-gray-400">
            储备利率 {(q.reserveIncome.returnRate * 100).toFixed(2)}%
            {liveRate && (
        ...
        <div className="rounded-lg bg-red-50 px-3 py-2">
          ...
          <p className="text-lg font-semibold text-red-600">-{formatUSD(q.reserveIncome.distributionCosts)}</p>
          <p className="text-xs text-gray-400">{q.reserveIncome.distributionPctOfReserve}% of 利息</p>
        </div>
        <div className="rounded-lg bg-green-50 px-3 py-2">
          <p className="text-xs text-gray-500">其他收入</p>
          <p className="text-lg font-semibold text-green-700">{formatUSD(q.otherRevenue.total)}</p>
...
function ProductsView() {
  ...
  const cctp = calcCCTPRevenue({
    totalVolume: LATEST_ATTRIBUTION.productVolumes.cctpVolume,
    fastTransferPct: 0.40,
    avgFastFeeBps: 2.0,
  })

  const cpn = calcCPNRevenue({
    annualizedTpv: LATEST_ATTRIBUTION.productVolumes.cpnAnnualizedTpv,
    avgFeeBps: 10,
  })

  const otherRev = LATEST_OTHER_REVENUE
  ...
  const productData = [
    ...
    { name: 'USYC 管理费', value: otherRev.subscription.components.usycManagementFees, fill: '#ff6f00', note: '管理费随 AUM 增长' },
  ]

  const pieData = productData
        ...
        <div className="space-y-2">
          ...
          <div className="pt-2 border-t border-gray-100 text-xs">
            <p className="font-medium text-gray-700">
              {LATEST_ATTRIBUTION.periodLabel} 其他收入合计: <span className="text-gray-900">${(LATEST_OTHER_REVENUE.total / 1e6).toFixed(1)}M</span>
            </p>
            <p className="text-gray-400 mt-1">
              占总收入 {(LATEST_OTHER_REVENUE.total / LATEST_ATTRIBUTION.totalRevenue * 100).toFixed(1)}%
              — 94%+ 仍来自储备利息
            </p>
          </div>
        </div>
...
function SensitivityView({ liveRate }: { liveRate: LiveRateData | null }) {
  const q = LATEST_ATTRIBUTION
  // Base rate: use live T-bill rate if available, otherwise the latest quarter's actuals
  const baseRate = liveRate?.rate ?? q.reserveIncome.returnRate
  const baseLabel = liveRate
    ? `当前 (${(baseRate * 100).toFixed(2)}%)`
    : `${q.periodLabel} (${(q.reserveIncome.returnRate * 100).toFixed(2)}%)`
  ...
  const data = scenarios.map(s => {
    const result = calcRateSensitivity({
      avgCirculation: q.reserveIncome.avgCirculation,
  ...
  return (
    <div className="my-4">
      <div className="text-xs bg-red-50 text-red-700 rounded px-3 py-2 mb-4">
        每降息 100bps，Circle 年净收入减少约 $293M。盈亏平衡利率约 2.0-2.5%。
        {liveRate
